...
function generateTimestamp(): string {
  ...
  return now.toISOString()
    .replace(/:/g, '-')
    .replace(/\..+/, '')
    .replace('T', '_'); // 2026-04-22_10-30-45
}
...
function resolveOutputPath(
  ...
  if (imagePath) {
    if (path.isAbsolute(imagePath)) {
      return imagePath; // 绝对路径直接使用
    }
    return path.join(workspace, imagePath); // 相对路径基于 workspace
  }

  // 2. 配置文件指定了默认输出目录
  const baseDir = defaultOutputDir
    ? (path.isAbsolute(defaultOutputDir) ? defaultOutputDir : path.join(workspace, defaultOutputDir))
    : path.join(workspace, 'media', 'baoyu-imagine');

  // 3. 自动生成路径
  const timestamp = generateTimestamp();
  const slug = prompt ? generateSlug(prompt, 30) : 'image';
  const taskId = `${timestamp}-${slug}`;
  const outputDir = path.join(baseDir, taskId);

  return path.join(outputDir, 'image.png');
}
...
Options:
  ...
  --image <path>            Output image path (optional, auto-generated if not specified)
...
Env file load order: CLI args > process.env > OpenClaw root .env`);
}
...
async function loadEnv(): Promise<void> {
  // 查找 OpenClaw 根目录的 .env
  let current = process.cwd();
  let openclawRoot: string | null = null;

  // 向上查找，直到找到包含 .env 的目录（假设是 OpenClaw 根目录）
  for (let i = 0; i < 10; i++) {
    const envPath = path.join(current, ".env");
    if (await exists(envPath)) {
      openclawRoot = current;
      break;
    }
  ...
  if (openclawRoot) {
    const openclawEnv = await loadEnvFile(path.join(openclawRoot, ".env"));
    for (const [k, v] of Object.entries(openclawEnv)) {
...
export async function loadExtendConfig(
  ...
  const configPath = path.join(cwd, ".baoyu", "baoyu-imagine.md");
...
export function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, AZURE_OPENAI_API_KEY+AZURE_OPENAI_BASE_URL, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, ZAI_API_KEY, MINIMAX_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, or ARK_API_KEY.\n" +
      "Create the project root .env file (preferred). Legacy .baoyu-skills/.env files are still read as fallback."
  );
}
...
async function prepareSingleTask(args: CliArgs, extendConfig: Partial<ExtendConfig>): Promise<PreparedTask> {
  ...
  // imagePath 现在可能为 null（自动生成路径）
  if (!args.imagePath) {
    throw new Error("imagePath should be resolved before prepareSingleTask");
  }
...
async function runSingleMode(args: CliArgs, extendConfig: Partial<ExtendConfig>): Promise<void> {
  ...
  const outputPath = resolveOutputPath(
    ...
    args.imagePath,
    args.prompt,
    extendConfig.default_output_dir ?? null,
  );
